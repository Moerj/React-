(function() {
	...
	var PublicMixin = {
		getRandomTransform: function() { // 取随机旋转角度(css3属性)
			...
			function randomInt(n) { // 取随机整数,范围值0~n
				return parseInt(Math.floor(Math.random() * n + 1));
			}
			return 'translate3d(' + randomInt(getMoveX()) * randomSign() + 'px,' +
				randomInt(getMoveY()) * randomSign() + 'px,0)' +
				' rotate3d(0,0,1,' + randomInt(180) * randomSign() + 'deg)';
		}
	};
	...
	var ImgComponent = React.createClass({
		...
		render: function() {
			...
			return (
				<div onClick={this.props.clickCallback} style={style} className={this.state.centerClass} data-index={this.props.index} ></div>
			)
		}
	});
